...
skf.get_n_splits(all_X, Y_train)
logging.basicConfig(format='%(asctime)s - %(message)s', level=logging.INFO, filename='SKFAccuracyPrecisionRecallFscoreBinary.log')
...
for train_index, test_index in skf.split(all_X, all_y):
    ...
    # Create the model
    model.fit(X_train, y_train, epochs=50, batch_size=250, verbose=1, validation_split=0.1)
    Infered = []
    testreshape = []
